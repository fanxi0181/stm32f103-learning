...
| 文件 | 启用开关 | 模式 |
|------|----------|------|
| My_Code1.h | `SIX_MODULE_LINKAGE_FREERTOS_ENABLED` | 单层 `#ifdef`（无 `#else` 空宏） |
...
| 问题 | 位置 | 说明 |
|------|------|------|
| `RX_DMA_BUFFER_SIZE` | My_Code3.h/c | 拼写已修正为 BUFFER` |
| `TEST_CPU_USAGE_ENABLE` | app_config.h | 拼写已修正为 ENABLED` |
